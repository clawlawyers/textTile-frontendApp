...
const EditClientScreen = ({navigation, route}: AddNewUserProps) => {
  const [clientName, setClientName] = useState(route.params.clientDetails.name);
  const [clientContact, setClientContact] = useState(
    route.params.clientDetails.phone,
  );
  const [clientEmail, setClientEmail] = useState(
    route.params.clientDetails.email,
  );
  const [firmName, setFirmName] = useState(route.params.clientDetails.firmName);
  const [firmAddress, setFirmAddress] = useState(
    route.params.clientDetails.address,
  );
  const [firmGST, setFirmGST] = useState(
    route.params.clientDetails.firmGSTNumber,
  );
  ...
  const addClientHandler = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${NODE_API_ENDPOINT}/clients/${route.params.clientDetails._id}`,
        {
          method: 'PUT',
          headers: {
      ...
      navigation.replace('ClientDetailsScreen', {
        clientId: route.params.clientDetails._id,
      });
    } catch (error) {
      setLoading(false);
      console.log(error);
