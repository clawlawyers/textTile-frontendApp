...
> = ({ navigation }) => {
    ...
    useCallback(() => {
      const getProductSalesAnalytics = async () => {
        ...
        try {
          ...
          const paidOrders = summary?.byStatus.paid.count || 0;
          const partialOrders = summary?.byStatus.partial.count || 0;
          const pendingOrders = summary?.byStatus.pending.count || 0;
          
          const totalOrders = paidOrders + partialOrders + pendingOrders;
          
          setAnalyticsData([
            { name: 'Orders Completed', value: paidOrders, color: '#292C33' },
            { name: 'Orders Active', value: partialOrders + pendingOrders, color: '#666666' },
            { name: 'Total Orders', value: totalOrders, color: '#ffffff' },
          ]);
        } catch (error) {
  ...
  const chartData = analyticsData.filter(item => 
    item.name === 'Orders Active' || item.name === 'Orders Completed'
  );
        ...
        <View className="space-y-3 gap-2">
          ...
          <View className="flex-row items-center rounded-xl overflow-hidden">
            <View className="bg-black w-1/2 px-4 py-4">
              <Text className="text-white font-semibold text-base">Total Orders </Text>
            </View>
          ...
          <View className="flex-row items-center rounded-xl overflow-hidden">
            <View className="bg-black w-1/2 px-4 py-4">
              <Text className="text-white font-semibold text-base">Orders Completed</Text>
            </View>
            <View className="bg-[#FBDBB5] w-1/2 px-4 py-4">
              <Text className="text-right text-base font-semibold text-black">
          ...
          <View className="flex-row items-center rounded-xl overflow-hidden mb-4">
            <View className="bg-black w-1/2 px-4 py-4">
              <Text className="text-white font-semibold text-base">Orders Active</Text>
            </View>
            <View className="bg-[#FBDBB5] w-1/2 px-4 py-4">
              <Text className="text-right text-base font-semibold text-black">
