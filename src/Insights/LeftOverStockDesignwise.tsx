...
const LeftoverstockDesignwiseScreen: React.FC<LeftoverstockDesignwiseScreenProps> = ({
  navigation,
}) => {
  ...
  const handleSearch = () => {
    if (!searchProduct.trim()) {
      console.log('Search empty, fetching all data');
      getProductSalesAnalytics();
      return;
    }
    console.log('Searching for designCode:', searchProduct);
    getProductSalesAnalytics();
  };

  const clearSearch = () => {
    setSearchProduct('');
    console.log('Search cleared, fetching all data');
    getProductSalesAnalytics();
  };
  ...
  const getProductSalesAnalytics = async () => {
    ...
    try {
      ...
      if (searchProduct.trim()) {
        url += `&designCode=${encodeURIComponent(searchProduct.trim())}`;
      }
